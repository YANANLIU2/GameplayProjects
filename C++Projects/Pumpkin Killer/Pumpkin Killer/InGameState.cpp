	...
	, one_kill_score_(20)
{
	...
	for (size_t y = 0; y < kTileRows; ++y)
	{
		for (size_t x = 0; x < kTileColumns; ++x)
		{
			...
			tiled_map_[y * kTileColumns + x] = new Tile(index, kTileColumns, kTileRows, x, y);
		}
	}
...
InGameState::~InGameState()
{
	...
	for (int y = 0; y < kTileRows; ++y)
	{
		...
		{
			delete tiled_map_[y * kTileColumns + x];
		}
	}
...
void InGameState::Draw()
{
	...
	for (int y = 0; y < kTileRows; ++y)
	{
		for (int x = 0; x < kTileColumns; ++x)
		{
			tiled_map_[y * kTileColumns + x]->DrawAll(hdcScreen, hdcMemory);
		}
	}
